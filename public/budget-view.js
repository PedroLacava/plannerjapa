(function () {
  ...
  function render() {
    ...
    function effectiveLegs(gotemba) {
    ...
    function groups(gotemba) {
      var legs=effectiveLegs(gotemba), acts=activityData.activities;
    ...
    function details(g,rate) {
      ...
      g.legs.forEach(function(x){
        ...
        lines.push('<li><span class="detail-kind">Transporte</span><b>'+esc(x[1])+' → '+esc(x[2])+'</b><span>'+esc(x[3])+' · '+yen(x[5])+' / '+brl(x[5],rate)+jr+'</span></li>');
      });
    ...
    function update(){
      var people=Math.max(1,Number(document.getElementById("budget-people").value)||1),rate=Math.max(1,Number(document.getElementById("budget-rate").value)||29),gotemba=document.getElementById("budget-gotemba").checked,category=document.getElementById("budget-category").value;
      var all=groups(gotemba), visible=all.filter(function(g){return category==="Todos"||g.categories.indexOf(category)!==-1;});
