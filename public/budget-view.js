(function () {
  ...
  function render() {
    ...
    function update(){
      ...
      var tt=all.reduce(function(s,g){return s+g.transportCost;},0),at=all.reduce(function(s,g){return s+g.activityCost;},0),stay=window.JaplannerReservations?window.JaplannerReservations.getAccommodationTotals(rate):{jpy:0,brl:0},stayPerson=stay.jpy/people,total=tt+at+stayPerson,groupTotal=(tt+at)*people+stay.jpy;
      host.querySelector("[data-budget-single]").textContent=yen(total);host.querySelector("[data-budget-single-brl]").textContent="≈ "+brl(total,rate);host.querySelector("[data-budget-group]").textContent=yen(groupTotal);host.querySelector("[data-budget-group-brl]").textContent="≈ "+brl(groupTotal,rate);host.querySelector("[data-budget-split]").textContent=yen(tt)+" + "+yen(at)+" + "+yen(stayPerson);host.querySelector("[data-budget-split-brl]").textContent="Transportes "+brl(tt,rate)+" · Passeios "+brl(at,rate)+" · Hospedagem "+brl(stayPerson,rate);
