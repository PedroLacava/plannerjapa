(function () {
  ...
  function render() {
    ...
    host.innerHTML = '<div class="transport-shell activity-shell"><div class="transport-hero"><div><div class="today-eyebrow">Orçamento completo</div><h2>Transportes e passeios</h2><p>Todos os deslocamentos e atrações do roteiro, organizados por dia em uma única tabela.</p></div><button type="button" data-close-budget>Voltar ao calendário</button></div>' +
      ...
      '<div class="transport-table-wrap"><table class="transport-table activity-table budget-table"><thead><tr><th>Data</th><th>Tipo</th><th>Item</th><th>Valor</th><th>Reserva ou passe</th><th>Observação</th></tr></thead><tbody data-budget-body></tbody></table></div></div>';
    ...
    function buildRows(gotemba) {
      ...
      activityData.activities.forEach(function (a) {
        rows.push({ date:a[0], type:"activity", title:a[1], subtitle:a[2], cost:a[3], action:a[6] === "yes" ? "Reservar" : "Não obrigatória", note:a[5], optional:a[4] === "Opcional", pending:a[4] === "Pendente" });
      });
